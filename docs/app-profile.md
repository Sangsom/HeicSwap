...
> **Minimum iOS**: 26.0
> **Status**: Planning (project scaffolded — tasks 1.1–1.3 + 2.1–2.2 + 3.1–3.5 complete; features not yet built)
...
├── Features/       # Home (hosts the import harness until 4.1), Settings (placeholder), Import (3.5 — PhotosImportButton, FilesImportButton, ImportView harness)
...
|-------|----------|-------|
| Placeholder shell | Low | `MainTabView` is a template placeholder; Home temporarily hosts the import harness (`ImportView`) so 3.5 can be exercised on device — both replaced by the Convert shell in task 4.1 |
