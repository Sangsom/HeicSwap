...
> **Minimum iOS**: 26.0
> **Status**: Planning (project scaffolded — tasks 1.1–1.3 + 2.1–2.2 + 3.1–3.4 complete; features not yet built)
...
├── Services/       # PurchaseService, AnalyticsService, ConversionEngine (3.1 — ImageIO transcode + bounded batch; 3.2 — maxDimension downscale + targetBytes quality search; 3.3 — opt-in EXIF/GPS/maker-note/TIFF strip on the write path), PDFBuilder (3.4 — image→multi-page PDF, one page per image, via UIGraphicsPDFRenderer streaming write + ImageIO downscale-on-load)
